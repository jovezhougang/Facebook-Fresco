...
import com.example.frescolibrary.R;
 ...
 * 
 * @attr ref com.facebook.R.styleable#GenericDraweeView_fadeDuration Images &
 *       scale types parameters:
 ...
 * @attr ref
 *       com.facebook.R.styleable#GenericDraweeView_placeholderImageScaleType
 ...
 * @attr ref
 *       com.facebook.R.styleable#GenericDraweeView_progressBarImageScaleType
 * @attr ref
 *       com.facebook.R.styleable#GenericDraweeView_progressBarAutoRotateInterval
...
public class GenericDraweeView extends DraweeView<GenericDraweeHierarchy> {
    ...
    private void inflateHierarchy(Context context, @Nullable AttributeSet attrs) {
        ...
        if (attrs != null) {
            ...
            try {
                ...
                retryImageId = gdhAttrs.getResourceId(
                        R.styleable.GenericDraweeView_retryImage, retryImageId);
                // retry image scale type
                retryImageScaleType = getScaleTypeFromXml(gdhAttrs,
                ...
                failureImageScaleType = getScaleTypeFromXml(gdhAttrs,
                ...
                progressBarAutoRotateInterval = gdhAttrs
                        .getInteger(
                                R.styleable.GenericDraweeView_progressBarAutoRotateInterval,
                                0);

                // actual image scale type
                actualImageScaleType = getScaleTypeFromXml(gdhAttrs,
                ...
                overlayId = gdhAttrs.getResourceId(
                        R.styleable.GenericDraweeView_overlayImage, overlayId);
        ...
        if (placeholderId > 0) {
            builder.setPlaceholderImage(resources.getDrawable(placeholderId),
                    placeholderScaleType);
        }
        if (retryImageId > 0) {
            builder.setRetryImage(resources.getDrawable(retryImageId),
                    retryImageScaleType);
        }
        if (failureImageId > 0) {
            builder.setFailureImage(resources.getDrawable(failureImageId),
                    failureImageScaleType);
        }
        if (progressBarId > 0) {
            Drawable progressBarDrawable = resources.getDrawable(progressBarId);
            if (progressBarAutoRotateInterval > 0) {
                progressBarDrawable = new AutoRotateDrawable(
                        progressBarDrawable, progressBarAutoRotateInterval);
            }
        ...
        if (pressedStateOverlayId > 0) {
            builder.setPressedStateOverlay(getResources().getDrawable(
                    pressedStateOverlayId));
        }
        ...
        if (roundAsCircle || roundedCornerRadius > 0) {
            ...
            if (roundingBorderColor != 0 && roundingBorderWidth > 0) {
                roundingParams.setBorder(roundingBorderColor,
                        roundingBorderWidth);
            }
    ...
    /**
     * Returns the scale type indicated in XML, or null if the special 'none'
     * value was found.
     */
    private static ScalingUtils.ScaleType getScaleTypeFromXml(TypedArray attrs,
            int attrId, ScalingUtils.ScaleType defaultScaleType) {
        String xmlType = attrs.getString(attrId);
        return (xmlType != null) ? ScalingUtils.ScaleType.fromString(xmlType)
                : defaultScaleType;
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        AspectRatioMeasure.updateMeasureSpec(mMeasureSpec, mAspectRatio,
                getLayoutParams(), getPaddingLeft() + getPaddingRight(),
